...
import io.github.aliothliu.marble.MarbleProperties;
...
import io.github.aliothliu.marble.domain.menu.*;
...
public class RbacMenuServiceImpl implements RbacMenuService {
    ...
    private List<MenuRepresentation> roots(String nameLike, Set<MenuId> inMenus) {
        String SQL = "SELECT menu.id AS id, menu.icon AS icon, menu.name, menu.sort AS sort, menu.page_id AS page_id , page.path AS path, page.target AS target FROM " + this.marbleProperties.getJpa().getMenuTableName() + " menu RIGHT JOIN ( SELECT descendant FROM " + this.marbleProperties.getJpa().getMenuPathTableName() + " GROUP BY descendant HAVING count(descendant) = 1 ) menu_path ON menu.id = menu_path.descendant LEFT JOIN " + this.marbleProperties.getJpa().getPageTableName() + " page ON menu.page_id = page.id WHERE 1=1 ";
    ...
    private List<MenuRepresentation> forest(Set<MenuId> inMenus) {
        ...
        String SQL = "SELECT menu.id AS id, menu.icon AS icon, menu.name AS name, menu.page_id AS page_id , page.path AS path, page.target AS target, menu_path.ancestor as ancestor FROM " + this.marbleProperties.getJpa().getMenuTableName() + " menu LEFT JOIN " + this.marbleProperties.getJpa().getMenuPathTableName() + " menu_path ON menu.id = menu_path.descendant LEFT JOIN " + this.marbleProperties.getJpa().getPageTableName() + " page ON menu.page_id = page.id WHERE menu_path.distance = 1 ";
        if (!inMenus.isEmpty()) {
    ...
    public MenuId append(String name, PageId pageId, String icon, MenuId parentId) {
        ...
        MarbleRegistry.eventPublisher().publish(new MenuCreated(this, newMenu.getMenuId(), pageId));
